...
public class RailNetwork {
	...
	public ArrayList<String> routeMinDistance(String origin, String destination) {
		...
		HashMap<String, String> parents = new HashMap<>(); // Value is the closest station to the Key (in direction o ->
															// d)
		...
		for (int i = 0; i < stationList.size(); i++) {
			...
			for (Station adj : stationList.get(nextShortest).getAdjacentStations().keySet()) {
				if (!stationList.get(adj.getName()).isMarked() && dist.get(nextShortest) != Integer.MAX_VALUE
						&& (dist.get(nextShortest)
								+ stationList.get(nextShortest).getAdjacentStations().get(adj)) < (dist
										.get(adj.getName()))) {
					dist.replace(adj.getName(),
							dist.get(nextShortest) + stationList.get(nextShortest).getAdjacentStations().get(adj)); // Update
																													// dist
																													// to
																													// reflect
																													// new
																													...
																													// this
																													// path
					parents.replace(adj.getName(), nextShortest); // Update parent to be the new best path to this
																	// station
	...
	public ArrayList<String> routeMinStop(String origin, String destination) {
		...
		HashMap<String, String> parents = new HashMap<>(); // Value is the station with least stops to the Key (in
															// direction o -> d)
		...
		for (int i = 0; i < stationList.size(); i++) {
			...
			// Updating distance of adjacent stations to picked station
			for (Station adj : stationList.get(nextShortest).getAdjacentStations().keySet()) {
				if (!stationList.get(adj.getName()).isMarked() && stops.get(nextShortest) != Integer.MAX_VALUE
						&& (stops.get(nextShortest) + 1) < (stops.get(adj.getName()))) {
					stops.replace(adj.getName(), stops.get(nextShortest) + 1); // Update dist to reflect new num of
																				// stops to this shortest path
					parents.replace(adj.getName(), nextShortest); // Update parent to be new min stops in shortest path
																	// to this station
					// Once we get to the destination, stop and return
					if (adj.getName().equals(destination)) {
						ArrayList<String> temp = getStops(parents, origin, destination);
